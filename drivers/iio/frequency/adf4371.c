...
static int adf4371_parse_dt(struct adf4371_state *st)
{
	...
	if (device_property_read_bool(&st->spi->dev, "adi,differential-ref-clock"))
	 ...
	 * bit to 0).
	 */
	if (device_property_read_bool(&st->spi->dev, "adi,loop-filter-inverting"))
		st->pd_pol = 0;
	else
		st->pd_pol = 1;

	ret = device_property_read_u32(&st->spi->dev, "adi,charge-pump-microamp", &tmp);
	if (ret < 0) {
	...
	ret = device_property_read_string_array(&st->spi->dev, "clock-output-names",
...
static int adf4371_probe(struct spi_device *spi)
{
	...
	if (device_property_read_bool(&spi->dev, "adi,chip-powerdown-and-exit-enable")) {
		...
		regmap_write(regmap, ADF4371_REG(0x25), 0);
		/* Disable ADC Clock, Power-Down N Divider */
		regmap_write(regmap, ADF4371_REG(0x73), 6);
		/* Power-Down */
		regmap_write(regmap, ADF4371_REG(0x1E), 4);
