...
static bool max_ser_format_valid(struct max_ser_priv *priv, u32 code)
{
	...
	return max_format_by_code(code);
}
...
static int max_ser_v4l2_notifier_register(struct max_ser_subdev_priv *sd_priv)
{
	...
	v4l2_async_nf_init(&sd_priv->notifier);

	mas = v4l2_async_nf_add_fwnode(&sd_priv->notifier,
				       sd_priv->slave_fwnode,
				       struct max_ser_asd);
	if (IS_ERR(mas)) {
		ret = PTR_ERR(mas);
	...
	ret = v4l2_async_subdev_nf_register(&sd_priv->sd, &sd_priv->notifier);
	if (ret) {
		dev_err(priv->dev,
...
error_cleanup_notifier:
	v4l2_async_nf_cleanup(&sd_priv->notifier);
...
error:
	v4l2_async_nf_unregister(&sd_priv->notifier);
	v4l2_async_nf_cleanup(&sd_priv->notifier);
